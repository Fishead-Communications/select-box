...
b("#sbSelector_"+e).text(l.text()));break;case 13:0<k.length&&h._changeSelectbox(g,k.attr("rel"),k.text());h._closeSelectbox(g);break;case 9:g&&(f=h._getInst(g))&&(0<k.length&&h._changeSelectbox(g,k.attr("rel"),k.text()),h._closeSelectbox(g));g=parseInt(d.attr("tabindex"),10);a.shiftKey?g--:g++;b("*[tabindex='"+g+"']").focus();break;case 27:h._closeSelectbox(g)}/[a-z0-9]/i.test(a.key)&&-1===p.indexOf(c)&&(b("a",d).removeClass(f.settings.classFocus),0<k.length&&(l=k.parent().nextAll("li:has(a)").eq(0).find("a[accesskey='"+
a.key+"']")),0===l.length&&(l=d.find("ul").find("a[accesskey='"+a.key+"']").eq(0)),0<l.length&&(l.addClass(f.settings.classFocus).focus(),b("#sbSelector_"+e).text(l.text())));a.stopPropagation();return!1}).delegate("a","mouseover",function(a){b(this).addClass(e.settings.classFocus)}).delegate("a","mouseout",function(a){b(this).removeClass(e.settings.classFocus)});r.appendTo(n);t.appendTo(n);n.insertAfter(f);b("html").on("mousedown",function(a){a.stopPropagation();b("select").selectbox("close")});
